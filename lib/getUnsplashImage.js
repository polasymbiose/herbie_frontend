...
const convertImageToWebP = (slug, name, ext) =>
  new Promise((resolve, reject) => {
    sharp(`/tmp/${slug}.png`).jpeg({
      quality: 60
    })
    .toFile(
      `${process.cwd()}/public/images/${name}${ext}`,
      (error, info) => {
        if (error) {
          reject(error);
        } else {
...
const getUnsplashImage = async (photo, ext, name) => {
  console.log('photo, ext, name', photo, ext, name)
  const resp = await fetch(
    `http://localhost:1337/uploads/${photo}${ext}`
  );
  let dimensions = null
  if (fs.existsSync(`${process.cwd()}/public/images/${name}${ext}`)) {
    
  } else {
    await temporaryStoreImage(photo, resp);
    await convertImageToWebP(photo, name, ext);
